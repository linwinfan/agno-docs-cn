...
You can customize the 代理's behavior by:
- **Adjusting Time Windows**: Modify the `calculate_start_date()` 函数 to analyze different time periods
- **Adding Sources**: Include additional websites or platforms in your analysis prompts
- **Modifying 密钥words**: Change 搜索 terms to focus on specific topics or industries
- **Customizing Output**: Modify the `expected_output` template to match your reporting needs

## 示例 Prompts

Here are some additional prompt 示例 you can try:
