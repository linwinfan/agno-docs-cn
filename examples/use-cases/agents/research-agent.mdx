...
Build an AI re搜索 代理 that combines web 搜索 with professional journalism to perform comprehensive re搜索, fact-check 信息, and deliver well-structured NYT-style articles.

## What You'll 学习

By building this 代理, you'll 理解:
- How 使用 web 搜索 工具s (DuckDuckGo) for multi-source re搜索
- How to extract and analyze content from web articles using Newspaper4k
- How to structure professional journalistic writing with clear 说明
- How to define output templates for consistent article formatting

## 使用 Cases

Build investigative journalism 工具s, re搜索 report generators, fact-checking systems, or content analysis platforms.

## How It Works

The 代理 follows a professional journalism re搜索 工作流:

1. **Re搜索**: Searches 10+ authoritative sources using DuckDuckGo
2. **Extract**: 使用s Newspaper4k to pull content from relevant articles
...
5. **Verify**: Ensures 所有 facts and attributions are accurate

The 代理 maintains journalistic standards of objectivity and includes proper sourcing throughout.
...
The 代理 will conduct comprehensive re搜索 on your query using DuckDuckGo to find multiple authoritative sources. It extracts content from relevant articles using Newspaper4k, cross-references 信息, and synthesizes findings into a professional NYT-style article.
...
- Modify the re搜索 topic to explore different subjects
- Adjust the number of sources 搜索ed for more comprehensive coverage
- Customize the `expected_output` template to match your preferred article style
- 探索 [DuckDuckGo Tools](/integrations/工具kits/搜索/duckduckgo) for advanced 搜索 options