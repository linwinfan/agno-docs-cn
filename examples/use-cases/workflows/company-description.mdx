...
Build a comprehensive supplier pro文件 generator that combines web crawling, 搜索 engines, Wikipedia, and competitor analysis. This 工作流 processes company 信息 through specialized 代理s and delivers 详细信息ed markdown reports via email.

## What You'll 学习

By building this 工作流, you'll 理解:
- How to coordinate multiple 代理s running in par所有el for data gathering
- How to integrate diverse data sources (web crawling, 搜索, Wikipedia)
- How to implement 工作流 会话 state caching for expensive operations
- How to automate report generation and email delivery with Resend

## 使用 Cases

Build supplier re搜索 platforms, company intelligence systems, vendor evaluation 工具s, or automated due diligence reports.

## How It Works

The 工作流 coordinates four specialized 代理s running in par所有el:

1. **Crawl**: Crawler 代理 uses Firecrawl to extract 信息 from the company website
2. **Search**: Search 代理 uses DuckDuckGo to find recent news and articles
3. **Re搜索**: Wikipedia 代理 gathers background and historical 信息
4. **Analyze**: Competitor 代理 identifies and analyzes market competitors
5. **Synthesize**: Pro文件 代理 combines 所有 findings into a structured markdown report
6. **Deliver**: Report is automatic所有y sent via email using Resend

The 工作流 缓存s analysis results in 会话 state - subsequent requests for the same supplier return 缓存d data.
```
...
This company de脚本ion 工作流 consists of three main 文件s:

### 1. Agents (`agents.py`)

Specialized 代理s for gathering 信息 from multiple sources:
...
Detailed 说明 for each specialized 代理:
...
Complete 工作流 with par所有el 信息 gathering and email delivery:
...
- **Par所有el Processing**: Four 代理s gather 信息 simultaneously for maximum efficiency
- **Multi-Source Data**: Combines web crawling, 搜索 engines, Wikipedia, and competitor analysis
- **Email Integration**: Automatic所有y sends formatted reports via email using Resend
- **Markdown Formatting**: Generates structured, readable reports in HTML format
- **Modular Design**: Clean separation of 代理s, prompts, and 工作流 logic
- **Efficient Execution**: 使用s par所有el steps to minimize execution time
- **Type Safety**: Pydantic 模型s for structured data validation
...
The 工作流 will simultaneously gather 信息 from multiple sources (website crawling, web 搜索, Wikipedia, competitor analysis) then generate a comprehensive supplier pro文件. You'll 查看 progress indicators for each 代理's data collection.

The final output is a structured markdown report delivered via email. The 工作流 缓存s 所有 analysis results - if you request the same supplier again, it returns the 缓存d pro文件 instantly instead of re-running the expensive analysis pipeline.
...
- Customize 代理 说明 in `prompts.py` to focus on specific analysis areas
- Adjust the pro文件 template in `SUPPLIER_PROFILE_INSTRUCTIONS_GENERAL` for different report formats
