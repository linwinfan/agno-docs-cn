...
此示例展示 how 创建 a collaboration 团队 that 所有ows multiple 代理s to work together on re搜索 topics using the `collaborate` mode. In Collaborate Mode, 所有 团队 成员 are given the same task and the 团队 leader synthesizes their outputs into a cohesive response.
