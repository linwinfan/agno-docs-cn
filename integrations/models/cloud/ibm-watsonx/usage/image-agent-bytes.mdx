...
This example shows how to use IBM WatsonX with vision 功能. It loads an image from a file and passes it to the 模型 along with a prompt. The 模型 can then analyze the image and provide relevant information.

Note: This example uses a vision-capable 模型 (`meta-llama/llama-3-2-11b-vision-instruct`) and requires a sample image file.