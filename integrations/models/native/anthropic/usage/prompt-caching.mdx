...
description: Learn how to use prompt caching with Anthropic 模型s and Agno.
---
...
You can read 更多功能 about prompt caching with Anthropic 模型s [here](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching).

## 使用方法

To use prompt caching in your Agno setup, pass the `cache_system_prompt` argument when initializing the `Claude` 模型:
...
Notice that for prompt caching to work, the prompt needs to be of a certain length. You can read 更多功能 about this on Anthropic's [docs](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching#cache-limitations).


## Extended cache

You can also use Anthropic's extended cache beta feature. This updates the cache duration from 5 minutes to 1 hour. To activate it, pass the `extended_cache_time` argument and the 以下 beta header:
