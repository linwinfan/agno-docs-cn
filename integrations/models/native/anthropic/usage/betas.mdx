---
title: Beta Features
description: Learn how to use Anthropic's beta 特性 with Agno.
---

Anthropic's `betas` are experimental 特性 extending the 功能 of Claude 模型s.

They are supported via the `betas` parameter when initializing the `Claude` 模型:
...
Some beta 特性 will require additional configuration. For example, the context management beta requires you to configure the Agent Skills feature will require you to specify which 文档 formats to work with.
...
You can check how to use each one of these beta 特性 in our docs.

Here are the ones supported 原生ly with Agno:
