...
Agno 支持 using Google Cloud Storage (GCS) as a 存储 backend for Workflows using the `GcsJsonDb` class. This 存储 backend stores session 数据 as JSON blobs in a GCS bucket.

## 使用方法

Configure your workflow with GCS 存储 to enable 云-based session persistence.
