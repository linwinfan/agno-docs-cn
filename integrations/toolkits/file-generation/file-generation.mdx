...
The 以下 智能体 will generate files in different formats based on user requests.
...
| Name                  | Description                                                    |
| --------------------- | -------------------------------------------------------------- |
| `generate_json_file`  | Generates a JSON file from 数据 (dict, list, or JSON string)  |
| `generate_csv_file`   | Generates a CSV file from tabular 数据                        |
| `generate_pdf_file`   | Generates a PDF 文档 from text content                    |
| `generate_text_file`  | Generates a plain text file from string content              |

## 开发者资源

- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/file_generation.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/file_generation_tools.py)
