---
title: Neo4j
description: The Neo4jTools toolkit enables 智能体s to interact with Neo4j graph 数据库s for querying and managing graph 数据.
sidebarTitle: Neo4j
keywords: [neo4j, database]
---

**Neo4jTools** enables 智能体s to interact with Neo4j graph 数据库s for querying and managing graph 数据.

## Prerequisites

The 以下 example requires the `neo4j` library.

```shell
pip install -U neo4j
```

You will also need a Neo4j 数据库. The 以下 example uses a Neo4j 数据库 running in a Docker container.

```shell
docker run -d -p 7474:7474 -p 7687:7687 --name neo4j -e NEO4J_AUTH=neo4j/password neo4j
```

Make sure to set the `NEO4J_URI` environment variable to the URI of the Neo4j 数据库.
...
The 以下 智能体 can interact with Neo4j graph 数据库s:
...
| `数据库`              | `Optional[str]`     | `None`                  | Specific 数据库 name to connect to.                        |
...
| --------------------- | ---------------------------------------------------------------- |
| `list_labels`         | List all node labels in the graph 数据库.                     |
| `list_relationships`  | List all relationship types in the graph 数据库.              |
| `get_schema`          | Get comprehensive schema information about the graph.           |
| `run_cypher`          | Execute Cypher queries on the graph 数据库.                   |



## 开发者资源

- 查看 [Tools Source](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/neo4j.py)
