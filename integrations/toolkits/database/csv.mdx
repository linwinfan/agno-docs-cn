...
The 以下 智能体 will download the IMDB csv file and allow the user to query it using a CLI app.
...
| `duckdb_connection`      | `Any`                    | `None`  | Specifies a connection instance for DuckDB 数据库 operations. |
| `duckdb_kwargs`          | `Dict[str, Any]`         | `None`  | A dictionary of keyword arguments for configuring DuckDB operations. |
| `enable_read_csv_file`   | `bool`                   | `True`  | Enables the functionality to read 数据 from specified CSV files. |
...
| `enable_query_csv_file`  | `bool`                   | `True`  | Enables the functionality to execute queries on 数据 within CSV files. |
...
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/csv_tools.py)
