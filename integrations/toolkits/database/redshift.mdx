---
title: Redshift
description: The RedshiftTools toolkit enables an Agent to interact with Amazon Redshift 数据 warehouses.
sidebarTitle: Redshift
keywords: [redshift, database]
---

**RedshiftTools** enable an Agent to interact with Amazon Redshift 数据 warehouses.

## Prerequisites

The 以下 example requires the `redshift_connector` library.
...
| `port`               | `int`           | `5439`   | Port for the 数据库 connection.                              |
| `数据库`           | `Optional[str]` | `None`   | Database name. Uses `REDSHIFT_DATABASE`.                       |
...
- 查看 [Example](/examples/basics/tools/database/redshift)
