---
title: ScrapeGraph
description: ScrapeGraphTools enable an Agent to extract structured 数据 from webpages, convert content to markdown, and retrieve raw HTML content.
---

**ScrapeGraphTools** enable an Agent to extract structured 数据 from webpages, convert content to markdown, and retrieve raw HTML content using the ScrapeGraphAI API.

The toolkit provides 5 core 功能:

1. **smartscraper**: Extract structured 数据 using natural language prompts
2. **markdownify**: Convert web pages to markdown format
3. **searchscraper**: Search the web and extract information
4. **crawl**: Crawl websites with structured 数据 extraction
...
The 以下 examples require the `scrapegraph-py` library.

```shell
pip install -U scrapegraph-py
```

Optionally, if your ScrapeGraph configuration or specific 模型s require an API key, set the `SGAI_API_KEY` environment variable:
...
The 以下 智能体 will extract structured 数据 from a website using the smartscraper tool:
...
<Note>查看 the [Startup Analyst example](/examples/use-cases/智能体s/startup-analyst-智能体) </Note>
...
| `enable_smartscraper`    | `bool`         | `True`  | Enable the smartscraper function for LLM-powered 数据 extraction.                                       |
| `enable_markdownify`     | `bool`         | `False` | Enable the markdownify function for webpage to markdown conversion.                                      |
| `enable_crawl`           | `bool`         | `False` | Enable the crawl function for website crawling and 数据 extraction.                                      |
| `enable_searchscraper`   | `bool`         | `False` | Enable the searchscraper function for web search and information extraction.                            |
| `enable_智能体ic_crawler` | `bool`         | `False` | Enable the 智能体ic_crawler function for automated browser actions and AI extraction.                    |
...
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `smartscraper`     | Extract structured 数据 from a webpage using LLM and natural language prompt. Parameters: url (str), prompt (str).                          |
| `markdownify`      | Convert a webpage to markdown format. Parameters: url (str).                                                                                  |
| `crawl`            | Crawl a website and extract structured 数据. Parameters: url (str), prompt (str), 数据_schema (dict), cache_website (bool), depth (int), max_pages (int), same_domain_only (bool), batch_size (int). |
| `searchscraper`    | Search the web and extract information. Parameters: user_prompt (str).                                                                        |
| `智能体ic_crawler`  | Perform automated browser actions with optional AI extraction. Parameters: url (str), steps (List[str]), use_session (bool), user_prompt (Optional[str]), output_schema (Optional[dict]), ai_extraction (bool). |
...
- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/scrapegraph.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/scrapegraph_tools.py)
- 查看 [Tests](https://github.com/agno-agi/agno/blob/main/libs/agno/tests/unit/tools/test_scrapegraph.py)