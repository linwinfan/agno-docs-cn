...
**BrightDataTools** provide comprehensive web scraping 功能 including markdown conversion, screenshots, search engine results, and structured 数据 feeds from various 平台s like LinkedIn, Amazon, Instagram, and 更多功能.

## Prerequisites

The 以下 examples require the `requests` library:
...
Extract structured 数据 from 平台s like LinkedIn, Amazon, etc.:
...
| `enable_web_数据_feed`      | `bool`         | `True`             | Enable the web_数据_feed function.                                                                            |
...
| `web_数据_feed`      | Retrieves structured 数据 from various sources like LinkedIn, Amazon, Instagram, etc. Parameters: `source_type` (str), `url` (str), `num_of_reviews` (Optional[int]).                     |
...
- `crunchbase_company` - Crunchbase company 数据
- `zoominfo_company_profile` - ZoomInfo company profiles
- `reuter_news` - Reuters news
- `github_repository_file` - GitHub repository files
- `yahoo_finance_business` - Yahoo Finance business 数据
## 开发者资源

- 查看 [Tools Source](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/bright数据.py)
- 查看 [Cookbook Example](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/bright数据_tools.py)