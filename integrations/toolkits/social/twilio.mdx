...
**TwilioTools** enables an Agent to interact with [Twilio](https://www.twilio.com/docs) 服务s, such as sending SMS, retrieving call details, and listing messages.

## Prerequisites

The 以下 examples require the `twilio` library and appropriate Twilio credentials, which can be obtained from [here](https://www.twilio.com/console).
...
The 以下 智能体 will send an SMS message using Twilio:
...
| `api_key`     | `Optional[str]` | `None`  | Twilio API Key for alter原生 authentication.    |
| `api_secret`  | `Optional[str]` | `None`  | Twilio API Secret for alter原生 authentication. |
...
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/twilio_tools.py)
