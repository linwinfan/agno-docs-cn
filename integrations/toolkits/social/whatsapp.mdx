...
This cookbook demonstrates how to use WhatsApp 集成 with Agno. Before running this example,
...
1. Create Meta Developer Account
   ...
   - Enable WhatsApp 集成 for your app [here](https://developers.facebook.com/docs/whatsapp/云-api/get-started)

2. Set Up WhatsApp Business API
   You can get your WhatsApp Business Account ID from [Business Settings](https://developers.facebook.com/docs/whatsapp/云-api/get-started)
...
- For first-time outreach, you must use pre-approved message templates
  [here](https://developers.facebook.com/docs/whatsapp/云-api/guides/send-message-templates)
...
For 更多功能 complex use cases, check out the WhatsApp Cloud API 文档ation:
[here](https://developers.facebook.com/docs/whatsapp/云-api/overview)

## Example

The 以下 智能体 will send a template message using WhatsApp:
...
| `async_mode`      | `bool`        | `False`   | Enable 异步 methods for sending messages.                                                            |
...
| `send_text_message_async`       | Sends a text message to a WhatsApp user (异步). Parameters: `text` (str), `recipient` (Optional[str]), `preview_url` (bool), `recipient_type` (str). |
| `send_template_message_async`   | Sends a template message to a WhatsApp user (异步). Parameters: `recipient` (Optional[str]), `template_name` (str), `language_code` (str), `components` (Optional[List[Dict[str, Any]]]). |

## 开发者资源

- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/whatsapp.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/whatsapp_tools.py)
