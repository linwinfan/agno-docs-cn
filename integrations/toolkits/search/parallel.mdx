...
The 以下 example requires the `parallel-web` library and an API key which can be obtained from [Parallel](https://平台.parallel.ai).
...
The 以下 智能体 will search for information on AI 智能体s and autonomous systems, then extract content from specific URLs:
...
| `mode` | `Optional[str]` | `None` | Default search mode. Options: "one-shot" or "智能体ic". |
...
- 查看 [Example](/examples/basics/tools/search/parallel)
- 查看 [Parallel SDK Documentation](https://docs.parallel.ai)
- 查看 [Parallel API Reference](https://docs.parallel.ai/api-reference)
