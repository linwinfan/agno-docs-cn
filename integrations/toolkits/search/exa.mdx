...
The 以下 examples require the `exa-py` library and an API key which can be obtained from [Exa](https://exa.ai).
...
The 以下 智能体 will search Exa for AAPL news and print the response.
...
| `use_autoprompt` | `Optional[bool]` | `None` | Enable autoprompt 特性 |
...
| `模型` | `Optional[str]` | `None` | Search 模型 to use ('exa' or 'exa-pro') |
...
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/exa_tools.py)
