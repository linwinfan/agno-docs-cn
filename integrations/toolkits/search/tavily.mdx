...
The 以下 examples requires the `tavily-python` library and an API key from [Tavily](https://tavily.com/).
...
The 以下 智能体 will run a search on Tavily for "language 模型s" and print the response.
...
| `search_depth`        | `Literal['basic', 'advanced']` | `'advanced'` | Depth of search - 'basic' for faster results or 'advanced' for 更多功能 comprehensive search.   |
| `format`              | `Literal['json', 'markdown']`  | `'markdown'` | Output format - 'json' for raw 数据 or 'markdown' for formatted text.                       |
...
| `web_search_with_tavily`  | Alter原生 search function that uses Tavily's search context API. Parameters include `query` (str) for the search query. Returns contextualized search results. Only available when `enable_search_context` is True. |

## 开发者资源

- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/tavily.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/tavily_tools.py)
