...
**E2BTools** enable an Agent to execute code in a secure sandboxed environment with support for Python, file operations, and web server 功能.
...
The 以下 example demonstrates how to create an 智能体 that can run Python code in a secure sandbox:
...
| `download_png_result`        | Add a PNG image result as an Image object to the 智能体  |
| `download_chart_数据`        | Extract chart 数据 from an interactive chart in results |
| `download_file_from_sandbox` | Download a file from the sandbox to the 本地 system    |
...
| Function         | Description                                             |
| ---------------- | ------------------------------------------------------- |
| `get_public_url` | Get a public URL for a 服务 running in the sandbox   |
...
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/e2b_tools.py)
