...
<Badge icon="code-branch" color="orange">
    <Tooltip tip="Introduced in v2.2.6" cta="查看 release notes" href="https://github.com/agno-agi/agno/releases/tag/v2.2.6">v2.2.6</Tooltip>
</Badge>

Notion is a powerful workspace management tool that allows you to create, organize, and collaborate on your work.

Agno provides tools for interacting with Notion 数据库s to help you automate your workflows and streamline your work.
...
1. Go to [https://www.notion.so/my-集成s](https://www.notion.so/my-集成s)
2. Click on **"+ New 集成"**
...
2. Add a **Database** (you can use "/数据库" command)
3. Set up the 数据库 with these properties:
...
1. Open your 数据库 page in Notion
2. Click the **"..."** (three dots) menu in the top right
3. Scroll down and click **"Add connections"**
4. Search for your 集成 name (e.g., "Agno Query Classifier")
5. Click on it to grant access

### Step 4: Get Your Database ID

Your 数据库 ID is in the URL of your 数据库 page:

```
https://www.notion.so/../{database_id}?v={view_id}
```

The `数据库_id` is the 32-character string (with hyphens) between the workspace name and the `?v=`.
...
The 以下 example demonstrates how to use `NotionTools` to create, update and search for Notion pages with specific tags.
...
| ----------------- | ---------------------------------------------------------------------------------------------------------- |
| `create_page(title, tag, content)`   | Creates a new page in the Notion 数据库 with a title, tag, and content.               |
| `update_page(page_id, content)`      | Adds content to an existing Notion page by appending a new paragraph.                   | 
| `search_pages(tag)`  | Searches for pages in the 数据库 by tag and returns matching results.                                 |
...
| ------------------- | ------ | ------- | -------------------------------------------------------- |
| `api_key` | `Optional[str]` | `None`  | Notion API key (集成 token). Uses `NOTION_API_KEY` env var if not provided.              |
| `数据库_id`     | `Optional[str]` | `None`  | The ID of the 数据库 to work with. Uses `NOTION_DATABASE_ID` env var if not provided. |
...
- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/notion.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/notion_tools.py)
