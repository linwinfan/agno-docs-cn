---
title: Trello
description: Agno TrelloTools helps to integrate Trello functionalities into your 智能体s, enabling management of boards, lists, and cards.
---

## Prerequisites

The 以下 examples require the `trello` library and Trello API credentials which can be obtained by 以下 Trello's developer 文档ation.

```shell
pip install -U trello
```

Set the 以下 environment variables:
...
The 以下 智能体 will create a board called `ai-智能体` and inside it create list called `todo` and `doing` and inside each of them create card called `create 智能体`.
...
The `list_boards` function accepts a `board_filter` argument with the 以下 options:
...
- 查看 [Cookbook Example](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/trello_tools.py)
