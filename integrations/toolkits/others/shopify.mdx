...
**ShopifyTools** enable an Agent to analyze sales 数据, product performance, and customer insights using the Shopify Admin GraphQL API.

## Prerequisites

The 以下 example requires a Shopify Admin API access token with appropriate scopes.
...
**Required API scopes:**
- `read_orders` (for order and sales 数据)
- `read_products` (for product information)
- `read_customers` (for customer insights)
- `read_analytics` (for analytics 数据)

## Example

The 以下 智能体 will analyze sales 数据 and provide insights.
...
- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/shopify.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/shopify_tools.py)
