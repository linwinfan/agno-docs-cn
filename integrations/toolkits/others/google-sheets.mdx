...
Set up the 以下 environment variables:
...
1. Go to Google Cloud Console (https://console.云.google.com)
...
The 以下 智能体 will use Google Sheets to read and update spreadsheet 数据.
...
| `update_sheet`          | Update 数据 in a Google Sheet. Parameters include `数据` (List[List[Any]]) for the 数据 to update, `spreadsheet_id` (Optional[str]) for the ID of the Google Sheet, and `range_name` (Optional[str]) for the range to update. Returns success or failure message. |
...
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/googlesheets_tools.py)