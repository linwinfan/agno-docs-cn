...
The 以下 example requires the `spotify` library and an API key from [Spotify](https://developer.spotify.com/).

The only external library needed for spotify is the `httpx`. Which is already a dependency of Agno.

## Example

The 以下 智能体 will search for songs using Spotify API.
...
| `get_track_recommendations`   | Get track recommendations based on seed 数据        |
...
- 查看 [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/spotify.py)
- 查看 [Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/14_tools/spotify_tools.py)
