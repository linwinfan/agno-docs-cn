...
title: 运行团队
sidebarTitle: 运行团队
description: 学习如何运行 Agno 团队。
keywords: [团队, 运行团队, 使用 agno 运行团队, 使用 agno 智能体运行团队]
---

通过调用 `Team.run()` 或 `Team.arun()` 来运行您的团队。以下是解释团队如何运行的流程图：
...
1. **前置钩子执行**（如果配置）在运行开始前执行验证或设置。
2. **推理智能体运行**（如果启用）来计划和分解任务。
3. **上下文构建**包括系统消息、用户消息、聊天历史、用户记忆、会话状态和其他输入。
4. **模型调用**使用准备好的上下文。
5. **模型决定**是否直接响应、调用提供的工具，或将请求委派给团队成员。
6. **如果发生委派**，成员智能体并发执行他们的任务（在 `async` 模式下）并将结果返回给团队负责人。团队负责人模型处理这些结果并可能进一步委派或响应。
7. **响应处理**并解析为 [`output_schema`](/basics/input-output/overview#structured-output)（如果提供）。
...
See the [Input & Output](/basics/input-output/overview) docs 有关更多信息, and to see how to use structured input and output with teams.
...
有关更多信息 see the [Sessions](/basics/sessions/overview) documentation.
...
有关更多信息 see the [Multimodal](/basics/multimodal) documentation.
...
有关更多信息 see the [Input & Output](/basics/input-output/overview) documentation.
