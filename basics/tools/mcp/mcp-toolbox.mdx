---
title: MCP 工具箱
sidebarTitle: MCP 工具箱
description: 学习如何在 Agno 中使用 MCPToolbox 连接到具有工具过滤功能的 MCP 数据库工具箱。
---
...
**MCPToolbox** 使智能体能够连接到 Google 的 [MCP 数据库工具箱](https://googleapis.github.io/genai-toolbox/getting-started/introduction/)并具有高级过滤功能。它扩展了 Agno 的 `MCPTools` 功能，可以按工具集或工具名称过滤工具，允许智能体仅加载它们需要的特定数据库工具。

## 先决条件

使用 MCPToolbox 您需要以下内容：

```bash
pip install toolbox-core
```

我们的默认设置还需要您安装 Docker 或 Podman，以运行示例的 MCP 工具箱服务器和数据库。

## 快速开始

使用我们功能完整的演示立即开始使用 MCPToolbox。
...
这将启动一个带有示例酒店数据的 PostgreSQL 数据库和一个 MCP 工具箱服务器，该服务器将数据库操作作为过滤工具公开。
