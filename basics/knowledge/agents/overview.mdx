...
title: 带有知识的代理
sidebarTitle: 概述
description: 理解知识以及如何在Agno代理中使用它
keywords: [knowledge, agent, agentic rag, agentic search]
---

**知识**存储特定领域的内容，可以添加到代理的上下文中以实现更好的决策。

<Note>
Agno有一个通用知识解决方案，支持多种形式的内容。

在[知识](/basics/knowledge/overview)文档中查看更多详细信息。
</Note>

代理可以在运行时**搜索**这些知识以做出更好的决策并提供更准确的响应。这种**按需搜索**模式称为智能RAG。

<Tip>
示例：假设我们正在构建一个Text2Sql代理，我们需要向代理提供表架构、列名、数据类型、示例查询等，以帮助它生成最佳的SQL查询。

将所有这些都放在系统消息中是不可行的，相反，我们将这些信息存储为知识，让代理在运行时查询它。

使用这些信息，代理然后可以生成最佳的SQL查询。这称为**动态少样本学习**。
