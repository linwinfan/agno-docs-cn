...
title: 什么是嵌入器？
sidebarTitle: 概述
description: 学习如何使用Agno中的嵌入器将复杂信息转换为向量表示。
keywords: [embedders, embeddings]
---

嵌入器将文本、图像和其他数据转换为捕获含义的向量（数字列表）。这些向量使得语义存储和搜索信息变得容易——因此您通过意图和上下文找到内容，而不仅仅是精确的关键词。

如果您正在构建诸如检索增强生成(RAG)、语义搜索、文档问答或代理长期记忆等功能，嵌入器是使这一切工作的基础。

### 为什么使用嵌入器？

- **比关键词更好的召回率**：它们理解含义，因此"如何重置我的密码？"能找到提到"更改PIN码"的文档。
- **将LLM锚定在您的数据中**：在回答时为模型提供可信的、特定领域的上下文。
- **扩展到大型知识库**：向量支持跨数千或数百万块的快速相似性搜索。
- **多语言检索**：许多嵌入器将不同语言映射到相同的语义空间。
