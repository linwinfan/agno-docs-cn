---
title: 搜索与检索
description: 了解代理如何智能搜索和检索知识库中的信息以提供准确、上下文相关的响应。
keywords: [search, retrieval, rag, agentic rag, agentic search, search]
---

当代理需要信息来回答问题时，它不会将所有内容都倾倒到提示中。相反，它只搜索最相关的片段。这种专注的方法是使知识驱动的代理既有效又高效的原因——他们在需要时得到所需的确切信息。

## 代理如何搜索知识

将代理的搜索过程想象成一个熟练的研究员，知道要寻找什么以及在哪里找到它：

<Steps>
  <Step title="查询分析">
    代理分析用户的问题以理解什么类型的信息会有帮助。
  </Step>
  <Step title="搜索策略">
    基于分析，系统制定一个或多个搜索（向量、关键词或混合）。
  </Step>
  <Step title="信息检索">
    知识库返回最相关的内容块。
  </Step>
  <Step title="上下文集成">
    检索到的信息与原始问题结合起来生成全面的响应。
  </Step>
</Steps>
