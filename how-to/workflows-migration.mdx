---
title: 迁移到 Workflows 2.0
sidebarTitle: Workflows 2.0 迁移指南
description: 了解如何迁移到 Workflows 2.0。
---

## 从 Workflows 1.0 迁移

Workflows 2.0 是一种全新的智能体自动化方法，需要从 Workflows 1.0 实现升级。它引入了一种新的、更灵活和更强大的构建工作流的方式。

### 主要差异

| Workflows 1.0     | Workflows 2.0     | 迁移路径                         |
| ----------------- | ----------------- | -------------------------------- |
| 仅线性            | 多种模式          | 根据需要添加 Parallel/Condition  |
| 专注于 Agent      | 混合组件          | 将函数转换为 Steps               |
| 有限的分支        | 智能路由          | 用 Router 替换 if/else           |
| 手动循环          | 内置 Loop         | 使用 Loop 组件                   |

### 迁移步骤

1. **评估当前工作流**：识别并行机会
2. **添加条件**：将 if/else 逻辑转换为 Condition 组件
3. **提取函数**：将自定义逻辑移动到基于函数的步骤
4. **启用流式传输**：用于基于事件的信息
5. **添加状态管理**：使用 `workflow_session_state` 进行数据共享

### 博客文章生成器工作流示例

让我们以一个示例来演示如何构建一个复杂的博客文章生成器，它结合了
网络研究能力和专业写作专业知识。该工作流采用多阶段
方法：
...
要将其转换为 **Workflows 2.0** 结构，我们可以将工作流分解为更小的步骤并遵循[开发指南](/basics/workflows/workflow-patterns/overview)。
或者为了简单起见，我们可以直接将 run 方法替换为单个自定义函数执行器，如[此处](/basics/workflows/workflow-patterns/fully-python-workflow)所述。
