...
Apps are tools like `FastAPI`, `PgVector`, `Streamlit`, `Jupyter`, `Django` that we define as python classes and run using `ag start` or `ag infra up`.
...
- [Install requirements on startup](/templates/infra-management/install)
